...
rag_pipe.add_component("llm", OllamaGenerator(model="zephyr", generation_kwargs={"num_predict": 500, "temperature": 0.9}))
rag_pipe.connect("prompt_builder", "llm")


def rag_pipeline_func(query: str):
    txt_files = [str(f) for f in Path("data/").glob("*.txt")]
    converted = TextFileToDocument().run(sources=txt_files)
    split = DocumentSplitter(split_by="sentence", split_length=5).run(documents=converted["documents"])
    result = rag_pipe.run({"prompt_builder": {"documents": split["documents"], "question": query}})
...
# ── Custom Function Caller component ─────────────────────────────────────────
    ...
    def run(self, messages: List[ChatMessage]):
        ...
        if last_message.tool_calls:
            ...
            for tool_call in last_message.tool_calls:
                ...
                tool_replies.append(ChatMessage.from_tool(tool_result=json.dumps(result), origin=tool_call))
...
chat_agent.add_component("message_collector", BranchJoiner(List[ChatMessage]))
chat_agent.add_component("generator", OllamaChatGenerator(
    model="llama3.2",
    generation_kwargs={"num_predict": 500, "temperature": 0.9},
    tools=[rag_tool, integration_tool]
))
chat_agent.add_component("function_caller", OllamaFunctionCaller(
    available_functions={
...
print("Haystack Chat Agent ready! Ask anything about Haystack.")
